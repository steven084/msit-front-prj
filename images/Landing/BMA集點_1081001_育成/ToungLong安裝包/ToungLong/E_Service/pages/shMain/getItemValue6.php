...
switch ($functionName) {
    case 'shMainBodyTag':
        ...
        $sql = "SELECT A.* FROM `sh_mdse` A
                    ...
                    WHERE B.communityID = " . $Value . " AND A.Name LIKE '%" . $Tag . "%' AND A.IsShelf = '是' ORDER BY A.HandlingFee DESC Limit " . $Limit . ",9;";
        $result = mysqli_query($conn, $sql) or die('MySQL 語法錯誤' . $sql);
            ...
            echo '<div class="col-lg-4 col-md-6 col-sm-6 col-6 mb-4">
                    <div class="card h-100">
                        <a href="#" data-toggle="modal" data-target="#viewModal' . $row_result[0] . '">
                            ...
                            <img class="rounded mx-auto d-none d-sm-block" src="' . $row_result[5] . '" alt="Third slide" style="max-width:100%;height:25vw;">
                            <img class="rounded mx-auto d-block d-sm-none" src="' . $row_result[5] . '" alt="Third slide" style="max-width:100%;height:40vw;">
                        </a>
    ...
    case 'shMainFootTag':
        $sql = "SELECT A.* FROM `sh_mdse` A
                    INNER JOIN `sh_mdseclass` B
                    ON A.`mdseclassID` = B.ID
                    WHERE B.communityID = " . $Value . " AND B.Name LIKE '%" . $Tag . "%' AND A.IsShelf = '是';";
        $result     = mysqli_query($conn, $sql) or die('MySQL 語法錯誤' . $sql);
        $rowCount   = mysqli_num_rows($result);
