...
switch ($functionName) {
    case 'shMainBodyTag':
        ...
        $sql = "SELECT A.* FROM `sh_mdse` A
                    ...
                    WHERE B.communityID = " . $Value . " AND B.ID = " . $Tag . " AND A.IsShelf = '是' ORDER BY A.HandlingFee DESC Limit " . $Limit . ",9;";
        $result = mysqli_query($conn, $sql) or die('MySQL 語法錯誤' . $sql);
    ...
    case 'shMainFootTag':
        $sql = "SELECT A.* FROM `sh_mdse` A
                    INNER JOIN `sh_mdseclass` B
                    ON A.`mdseclassID` = B.ID
                    WHERE B.communityID = " . $Value . " AND B.ID = " . $Tag . " AND A.IsShelf = '是';";
        $result     = mysqli_query($conn, $sql) or die('MySQL 語法錯誤' . $sql);
        $rowCount   = mysqli_num_rows($result);
