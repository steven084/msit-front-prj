...
// -----------------------------------------------
    for ($i = 0; $i < Count($checkedRowsValueTemp2); $i++) {
        ...
        if ($date->getTimestamp() > $date2->getTimestamp()) {
            ...
            $sql2    = "INSERT INTO `es_houpaystatus` (`ID`, `PayPeriodTime`, `PayTime`, `PayMoney`, `PayDisMoney`, `Remark`, `Maintainer`, `ChangeDay`, `houpayitemID`) VALUES (Null, '" . $row_result[1] . "', '" . $DateTime . "', '" . $row_result[2] . "', '" . $row_result[3] . "','','" . $Maintainer . "','" . $DateTime . "', " . $row_result[0] . ");";
            $result2 = mysqli_query($conn, $sql2) or die('MySQL 語法錯誤' . $sql2);
