...
if ($HouseSelect == "All") {
    $sql = "SELECT A.*, B.ItemName,B.ItemCode,C.HouseNum,C.Name
            ...
            WHERE (A.ChangeDay BETWEEN '" . $StratTime . "' AND '" . $EndTime . "') AND A.buildingID = " . $BuildingID . "
    ...
    $sql = "SELECT A.*, B.ItemName,B.ItemCode,C.HouseNum,C.Name
            ...
            INNER JOIN es_household C
            ON A.householdID = C.ID AND A.buildingID = C.buildingID
            WHERE (A.ChangeDay BETWEEN '" . $StratTime . "' AND '" . $EndTime . "') AND A.buildingID = " . $BuildingID . " AND A.householdID = " . $HouseSelect . "
            AND A.ID NOT IN (SELECT houpayitemID FROM es_houpaystatus);";
}
